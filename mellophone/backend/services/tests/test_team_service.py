...
class TeamServiceTestCase(TestCase):
    ...
    def test_create_team_for_new_user(self):
        ...
        self.assertEqual(
            team.name,
            name,
            'The team should have a matching name'
        )
        self.assertEqual(
            team.website,
            website,
            'The team should have a matching website'
        )
        self.assertTrue(
        ...
        self.assertEqual(
            len(teams_of_user),
            1,
            "The user should only be a member of their new team"
        )
        self.assertEqual(
            teams_of_user[0].id,
            team.id,
    ...
    def test_user_not_in_team(self):
        ...
        self.assertTrue(self.team_service.is_user_in_team_with_id(
            user_in_team, team.id
        ), "The user who created a team should be a member")
        self.assertFalse(self.team_service.is_user_in_team_with_id(
            user_not_in_team, team.id
    ...
    def test_get_team(self):
        ...
        self.assertEqual(
            team.id,
            retrieved_team.id,
            'The retrieved team should have the expected ID'
        )
        self.assertEqual(
            team.name,
            retrieved_team.name,
